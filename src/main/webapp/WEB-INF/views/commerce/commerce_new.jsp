...
<div class="wrap">
    ...
    <div class="feed_up">
        ...
        <nav class="btn_group">
            <button id="btn_feed" onclick="location.href='/sns/feed'">
                <h3>피드</h3>
            </button>
    ...
    <c:forEach var="item" items="${newImageList}" begin="0" step="1" end="2" varStatus="status">
             <c:if test="${not empty newImageList[0].sns_imageFileName && newImageList[0].sns_imageFileName!='null'}">
              <input  type= "hidden"   name="originalFileName" value="${item.sns_imageFileName}" />
                <a class="img_link"> <img src="${contextPath}/download?sns_articleNO=${item.sns_articleNO}&sns_imageFileName=${item.sns_imageFileName}" width="122px" height="150px" onClick="location.href='/sns/feed?articleNO=${item.sns_articleNO}'" id="preview"/></a>
                </c:if>
              </c:forEach>
     </div>
