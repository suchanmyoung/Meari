...
<div class="wrap">
   ...
   <div class="feed_up">
        ...
        <nav class="btn_group">
            <!-- 버튼 50%로 크기 조절 -->
            <button id="btn_feed" onclick="location.href='/profile'">
                <h3>사진</h3>
            </button>
            <button id="btn_popular" style="color:rgb(1,199,60)">
                <h3>판매물건</h3>
            </button>
        </nav>
    ...
    <div class="feed_area">
        <div class="img_sort">
         <c:forEach var="item" items="${newImageList}" begin="0" step="1" end="2" varStatus="status">
             <c:if test="${not empty newImageList[0].commerce_imageFileName && newImageList[0].commerce_imageFileName!='null'}">
              <input  type= "hidden"   name="originalFileName" value="${item.commerce_imageFileName}" />
                <a class="img_link"> <img src="${contextPath}/profile_download?commerce_articleNO=${item.commerce_articleNO}&commerce_imageFileName=${item.commerce_imageFileName}" width="122px" height="122px" onClick="location.href='/commerce/detail?articleNO=${item.commerce_articleNO}'" id="preview"/></a>
                </c:if>
              </c:forEach>
     </div>
  ...
  <div class="img_sort">
    ...
    <c:forEach var="item" items="${newImageList}" begin="12" step="1" end="14" varStatus="status">
             <c:if test="${not empty newImageList[0].commerce_imageFileName && newImageList[0].commerce_imageFileName!='null'}">
              <input  type= "hidden"   name="originalFileName" value="${item.commerce_imageFileName}" />
                <a class="img_link"> <img src="${contextPath}/profile_download?commerce__articleNO=${item.commerce_articleNO}&commerce_imageFileName=${item.commerce_imageFileName}"  width="122px" height="122px" onClick="location.href='/commerce/detail?articleNO=${item.commerce_articleNO}'" id="preview"/></a>
                </c:if>
              </c:forEach>
              </div>
              <div class="img_sort">
